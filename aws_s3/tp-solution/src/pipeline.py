...
from src.scrapers import EcommerceScraper, Product
...
class EcommercePipeline:
    ...
    def __init__(self):
        ...
        self.stats = {"products_scraped": 0, "images_stored": 0, "errors": []}
    ...
    def process_product(
        self, product: Product, download_image: bool = True
    ) -> Optional[dict]:
        """
        ...
        try:
            ...
            if download_image and product.image_url:
                ...
                if product.image_data:
                    ...
                    minio_ref = self.minio.upload_image(product.image_data, image_path)
    ...
    def run(
        ...
        try:
            ...
            for category, subcategory, url in urls:
                logger.info(
                    "processing_category", category=category, subcategory=subcategory
                )

                # Collecter les produits
                products = list(
                    self.scraper.scrape_category(
                        url=url,
                        category=category,
                ...
                if show_progress:
                    iterator = tqdm(
                        products, desc=f"Processing {subcategory}", unit="product"
                    )
                else:
                    iterator = products
    ...
    def print_analytics(self) -> None:
        ...
        for cat in analytics.get("by_category", []):
            print(
                f"   {cat['category']}: {cat['count']} produits, "
        ...
        for i, p in enumerate(analytics.get("value_ranking", [])[:5], 1):
            print(f"   {i}. {p['title'][:40]}... (${p['price']:.2f}, {p['rating']}★)")
        ...
        for bucket, info in storage.items():
            print(
                f"   {bucket}: {info['count']} fichiers, {info['total_size_mb']:.2f} MB"
            )
...
Exemples:
  ...
  python -m src.pipeline --no-images --pages 10
    ...
    parser.add_argument(
        "--no-images", action="store_true", help="Ne pas télécharger les images"
    )
    parser.add_argument(
        "--export-csv", action="store_true", help="Exporter en CSV après le scraping"
    )
    parser.add_argument(
        "--export-json", action="store_true", help="Exporter en JSON après le scraping"
    )
    parser.add_argument(
        "--export-parquet",
        action="store_true",
    ...
    parser.add_argument(
        "--backup", action="store_true", help="Créer un backup après le scraping"
    )
    parser.add_argument(
        "--analytics",
        action="store_true",
