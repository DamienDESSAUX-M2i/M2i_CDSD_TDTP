...
class MinIOStorage:
    ...
    def upload_image(
        self, image_data: bytes, object_name: str, content_type: str = "image/jpeg"
    ) -> Optional[str]:
        """
    ...
    def get_image(self, object_name: str) -> Optional[bytes]:
        ...
        try:
            response = self.client.get_object(
                bucket_name=minio_config.bucket_images, object_name=object_name
            )
            data = response.read()
            response.close()
    ...
    def list_images(self, prefix: str = "") -> list[dict]:
        ...
        try:
            objects = self.client.list_objects(
                bucket_name=minio_config.bucket_images, prefix=prefix, recursive=True
            )
            return [
                {
    ...
    def list_exports(self) -> list[dict]:
        ...
        try:
            objects = self.client.list_objects(
                minio_config.bucket_exports, recursive=True
            )
            return [
                {
                    "name": obj.object_name,
    ...
    def list_backups(self) -> list[dict]:
        ...
        try:
            objects = self.client.list_objects(
                minio_config.bucket_backups, recursive=True
            )
            return [
                {
                    "name": obj.object_name,
    ...
    def get_presigned_url(
        self, object_name: str, bucket: str = None, expires_hours: int = 24
    ) -> Optional[str]:
        """
