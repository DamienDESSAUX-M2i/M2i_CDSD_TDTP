...
class MongoDBStorage:
    ...
    def _create_indexes(self) -> None:
        ...
        try:
            ...
            self.products.create_index([("category", ASCENDING), ("price", ASCENDING)])

            # Index pour l'historique des prix
            self.price_history.create_index([("sku", ASCENDING), ("date", DESCENDING)])
    ...
    def get_products_by_price_range(
        ...
        return self.find_products({"price": {"$gte": min_price, "$lte": max_price}})
    ...
    def get_products_by_rating(self, min_rating: int) -> list[dict]:
        ...
        return self.find_products(
            {"rating": {"$gte": min_rating}}, sort=[("rating", DESCENDING)]
        )
    ...
    def get_top_rated(self, limit: int = 10, category: str = None) -> list[dict]:
        ...
        return self.find_products(
            query, sort=[("rating", DESCENDING), ("price", ASCENDING)], limit=limit
        )
    ...
    def record_price(self, sku: str, price: float) -> None:
        ...
        self.price_history.insert_one(
            {"sku": sku, "price": price, "date": datetime.utcnow()}
        )
