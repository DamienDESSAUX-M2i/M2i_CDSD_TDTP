...
def main():
    ...
    print("\nPremières lignes:")
    print(
        df[["title", "price", "rating", "category", "subcategory"]].head(10).to_string()
    )
    ...
    axes[0].axvline(
        ...
        color="red",
        linestyle="--",
        label=f"Moyenne: ${df['price'].mean():.2f}",
    )
    axes[0].legend()
    ...
    axes[0].bar(
        rating_counts.index, rating_counts.values, color="gold", edgecolor="black"
    )
    ...
    if "subcategory" in df.columns:
        avg_rating = (
            df.groupby("subcategory")["rating"].mean().sort_values(ascending=True)
        )
        axes[1].barh(
            avg_rating.index, avg_rating.values, color="skyblue", edgecolor="black"
        )
    ...
    if "subcategory" in df.columns:
        cat_stats = (
            ...
            .agg({"price": ["count", "mean", "min", "max"], "rating": "mean"})
            .round(2)
        )
