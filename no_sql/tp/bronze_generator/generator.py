...
def generate_campaign():
    ...
    if random.random() > 0.45:
        campaign["target_audience"] = random.choice(
...
def main():
    ...
    for i in range(0, total_customers, batch_size):
        customers = [
            generate_customer() for _ in range(min(batch_size, total_customers - i))
        ]
    ...
    for i in range(0, total_transactions, batch_size):
        transactions = [
            ...
            for _ in range(min(batch_size, total_transactions - i))
        ]
    ...
    for i in range(0, total_campaigns, batch_size):
        campaigns = [
            generate_campaign() for _ in range(min(batch_size, total_campaigns - i))
        ]
    ...
    print("\nGénération terminée!")
    print(
        f"Total: {customers_collection.count_documents({})} clients, "
