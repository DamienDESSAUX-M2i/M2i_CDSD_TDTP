...
def exercise_2():
    ...
    cheap_laptops = mongo.find_products(
        {"subcategory": "laptops", "price": {"$lt": 500}}
    )
    ...
    most_expensive_by_cat = list(
        mongo.products.aggregate(
            [
                {"$sort": {"price": -1}},
                {
                    "$group": {
    ...
    for item in most_expensive_by_cat:
        ...
        print(
            f"  - {item.get('category', 'N/A')}: {title}... (${item.get('max_price', 0)})"
        )
    ...
    avg_price_good_rating = (
        avg_price_pipeline[0] if avg_price_pipeline else {"avg_price": 0, "count": 0}
    )
            ...
            [
                {"$match": {"price": {"$gt": 0}, "rating": {"$gt": 0}}},
                {
                    "$project": {
                        ...
                        "category": 1,
                        "value_score": {
                            "$divide": ["$rating", {"$divide": ["$price", 100]}]
                        },
                    }
                },
                {"$sort": {"value_score": -1}},
    ...
    for bucket in price_ranges:
        range_label = (
            f"${bucket['_id']}" if isinstance(bucket["_id"], int) else bucket["_id"]
        )
        print(
            f"  - {range_label}: {bucket['count']} produits, "
            ...
            [
                {
                    "$group": {
                        ...
                        "count": {"$sum": 1},
                        "products": {
                            "$push": {"title": "$title", "category": "$category"}
                        },
                    }
                },
                {"$match": {"count": {"$gt": 1}}},
                {"$sort": {"count": -1}},
                {"$project": {"_id": 0, "price": "$_id", "count": 1, "products": 1}},
            ]
        )
    )
